...
class UpdatePriceShell extends Shell {
    ...
    function main() {
        ...
        if ($allDbPharses == array()) {
            ...
            $this->out("!!!!!!!!!!! no data from DB !!!!!!!!!!!!!!!!!!!!!!");

            $doneAt = "done at  " . date('d-m-Y:H.i.s');
            $this->out("-------------------------------------------------\n");

            CakeLog::write('updateRes', $startAt . ' | no data from DB | ' . $timeRes . ' sek, | ' . $doneAt);
            return;
        }
        ...
        if ($resAllClients == array() || !isset($resAllClients['data'])) {
            ...
            $timeRes = "Time taken = " . number_format(($End - $Start), 2);
            $this->out($timeRes . " secs\n");
            $this->out("!!!!!!!! clients not returned from yandex !!!!!!!!!!!!!");

            $doneAt = "done at  " . date('d-m-Y:H.i.s');
            $this->out("-------------------------------------------------\n");

            CakeLog::write('updateRes', $startAt . ' | clients not returned from yandex | ' . $timeRes . ' sek, | ' . $doneAt);
        ...
        if ($resAllCampaigns == array() || !isset($resAllCampaigns['data'])) {
            $this->out("res All Campaigns = array()");
            $End = $this->getTime();
            $timeRes = "Time taken = " . number_format(($End - $Start), 2);
            $this->out($timeRes . " secs\n");
            $this->out("!!!!!!!!!!!!! campaigns not returned fro yandex !!!!!!!!!!!!");

            $doneAt = "done at  " . date('d-m-Y:H.i.s');
            $this->out("-------------------------------------------------\n");

            CakeLog::write('updateRes', $startAt . ' | campaigns not returned fro yandex | ' . $timeRes . ' sek, | ' . $doneAt);
            return;
        }
        ...
        if ($resAllBanners == array()) {
            $this->out("res All Banners = array()");
            $End = $this->getTime();
            $timeRes = "Time taken = " . number_format(($End - $Start), 2);
            $this->out($timeRes . " secs\n");
            $this->out("!!!!!!!!!!!! banners not returned for yandex !!!!!!!!!!!!");

            $doneAt = "done at  " . date('d-m-Y:H.i.s');
            $this->out("-------------------------------------------------\n");

            CakeLog::write('updateRes', $startAt . ' | banners not returned for yandex | ' . $timeRes . ' sek, | ' . $doneAt);
            return;
        }
...
//                        }                   
                ...
                foreach ($resAllPhrasesTmp as $k14=> $v14){
                    ...
                    $resAllPhrases['data'] = array_merge($resAllPhrases['data'], $v14['data']);
                }
       ...
       // $resAllPhrases = json_decode($getYnData->getYnData($pathToCerts, 'GetBannerPhrasesFilter', $params3), TRUE);
        if (!isset($resAllPhrases['data'])) {
            ...
            $timeRes = "Time taken = " . number_format(($End - $Start), 2);
            $this->out($timeRes . " secs\n");
            $this->out("!!!!!!!!!! phrases not returned for yandex !!!!!!!!!!!");
